...
mkdir -p $tmpdir $directory/backup/bin
cd $tmpdir
...
chmod -f +x "$tmpdir/mvdsv" 2> /dev/null
chmod -f -x "$directory/mvdsv" 2> /dev/null
chmod 644 "$tmpdir/fortress/qwprogs.dat" 2> /dev/null
chmod -f +x "$tmpdir/qtv/qtv.bin" 2> /dev/null
chmod -f -x "$directory/qtv/qtv.bin" 2> /dev/null
chmod -f +x "$tmpdir/qwfwd/qwfwd.bin" 2> /dev/null
