...
# nQWTFsv config update bash script (for Linux)
# by Empezar
...
echo
echo "Welcome to the nQWTFsv config updater"
echo "====================================="
echo
...
# Download maps
echo "=== Downloading ==="
wget --inet4-only -O qwtf-sv-configs.zip $mirror/qwtf-sv-configs.zip

# Terminate installation if not all packages were downloaded
if [ -s "qwtf-sv-configs.zip" ]
then
        if [ "$(du qwtf-sv-configs.zip | cut -f1)" \> "0" ]
        then
                echo foo >> /dev/null
        else
...
unzip -qqo qwtf-sv-configs.zip 2> /dev/null;echo "done"
...
do
        if [ -f "$file" ]
        then
                awk '{ sub("\r$", ""); print }' $file > /tmp/.nqwtfsv.tmp
                mv /tmp/.nqwtfsv.tmp $file
        fi
done
echo "done"
